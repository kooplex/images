...
if [ -z "$NEWGROUP" ] ; then
  ...
  exec su $NB_USER -c "cd /v ; sg $NEWGROUP -c \"env PATH=$PATH jupyter notebook-kooplex $* --NotebookApp.iopub_data_rate_limit=1.0e10 \" --EnvironmentKernelSpecManager.display_name_template=\" {}\" "
fi
