...
class FaerunWeb:
    ...
    def index(self, **params) -> IO:
        ...
        """
        import os, re
        prefix = os.getenv("NB_TESTURL")
        ls = []
        with open("/opt/conda/lib/python3.7/site-packages/faerun/assets/index_static.html") as f:
              for l in f:
                if re.match(r".*await fetch.*", l):
                    l = re.sub(r"(.*let response = await fetch.\'/)([a-z_]+\', {)", f"\\1{prefix}/\\2", l )
                ls.append(l)
#        print(ls)
        return "\n".join(ls)
